define(function(require, exports, module){
  return function(jquery){
    (function($) { // Localise the $ function
        ...
        get_time: function () {
            var that = this;
            $('.select_item_hour span').unbind('click').bind('click',function () {
        ...
        change_select_date: function () {
            var now_val = this_target.val();
            if (now_val) {
                var timeString = this.get_set_time(event);
                var dataString = now_val.split(' ')[0];
                this_target.val(dataString + timeString).change();
            }
        },
        ...
        show: function(e) {
			//创建了一个全局变量，用来获取当前的jdpicker元素
			this_target=$(e.target);
		   var $val=$(e.target).val();
		   if($val==''){
			   this.now_time();
		   }else{
		       var time_all=$val.split(' '),
				   time_arr=time_all[1].split(':');
			 $('.data_time_hour span').text(time_arr[0]);
			 $('.data_time_minute span').text(time_arr[1]);
			 $('.data_time_second span').text(time_arr[2]);
		   }
