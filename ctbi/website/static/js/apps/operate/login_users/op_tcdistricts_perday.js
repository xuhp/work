...
$(function () {
    ...
    var op_game_users_perday = {
        ...
        _data_show: function () {
            $header.delegate('.data_search_btn', 'click', function () {
                ...
                var this_url = url.domain + url.port + interFace.login_tcdistrictusers_perday;
                var json_data = {
                    'districtid': curid,
                    'date': date
                }
    ...
    function compare(a,b) {
        return a.Trange - b.Trange;
    }
    //获取显示数据
    function get_show_data(data) {
        var full_data = [];
        //创建空数据
        for (var i = 0; i < 24; i++) {
            full_data[i] = {
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            full_data[data[i].Trange - 1] = data[i];
        }
        return full_data;
    }
    //回调函数
