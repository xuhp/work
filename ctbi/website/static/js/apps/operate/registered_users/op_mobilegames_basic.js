...
$(function () {
    ...
    function search_byday() {
        ...
        var this_url = url.domain + url.port + interFace.reg_mobilegameusers_byday;
        var json_data = {
            'gameid': curid,
            'begin': begin,
            'end': end
    ...
    function search_bymonth() {
        ...
        var this_url = url.domain + url.port + interFace.reg_mobilegameusers_bymonth;
        var json_data = {
            'gameid': curid,
            'begin': begin,
            'end': end
    ...
    function callbackfn_bymonth(data) {
        var origin_data = get_show_data(data.sort(compare), 'month');
        if (origin_data.length == 0) {
            $data_show.empty().append('<div class="no_data">对不起，您搜索的时间段内没有数据！</div>');
            download_origin_data = null;
        } else {
