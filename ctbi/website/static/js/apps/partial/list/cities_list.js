...
function get_search_data(row_data, $val) {
    ...
    for (var i = 0, len = row_data.length; i < len; i++) {
        if (row_data[i].values.ShowName.indexOf($val) >= 0 || row_data[i].values.CityID==$val) {
    ...
    for (var i = 0, data_len = data.length; i < data_len; i++) {
        row_data[i] = {
            'values': {
                ...
                'ShowName': data[i].ProvinceName + '-' + data[i].CityName
            }
        }
    }
    return row_data;
...
//获取应用title
function get_app_title(data, cityid) {
    for (var i = 0, data_len = data.length; i < data_len; i++) {
        if (data[i].CityID == cityid) {
            return (data[i].ProvinceName+'-'+data[i].CityName)
        }
    }
}
